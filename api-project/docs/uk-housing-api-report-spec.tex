...
\subsection{Computed Indexes}

The affordability Index is the endpoint which both reads data and computes a derived result for the user based on user input:
\begin{itemize}[nosep]
    \item \textbf{Input:} Required salary and options to add their region name/id.
    \item \textbf{Data fetched:} Latest row in \texttt{rental\_metrics} for the region ($period$, $rental\_price\_all$) and the Region row from \texttt{regions} ($implied\_avg\_borrower\_income$, $implied\_income\_year$).
    \item \textbf{Computation:} 
        \[ monthly\_rent = rental\_price\_all \]
        \[ annual\_rent = monthly\_rent \times 12 \]
        \[ rent\_to\_salary\_ratio = \frac{annual\_rent}{salary} \]
        \[ rent\_to\_salary\_percent = ratio \times 100 \]
        If region implied income is present:
        \[ rent\_to\_region\_income\_percent = \left( \frac{annual\_rent}{implied\_avg\_borrower\_income} \right) \times 100 \]
    \item \textbf{Data representation:} The system returns a categorical affordability band derived from the calculated rent to salary ratio:
        \[
        \text{Affordability Band} = 
        \begin{cases} 
        \text{Affordable} & \text{if } rent\_to\_salary\_ratio \le 0.30 \\
        \text{Stretched} & \text{if } 0.25 < rent\_to\_salary\_ratio \le 0.4 \\
        \text{Unaffordable} & \text{if } rent\_to\_salary\_ratio > 0.4
        \end{cases}
        \]
        The final response is delivered as a JSON object containing the numerical percentages and the corresponding status string.
\end{itemize}

\subsection{Gemini API usage}

The system integrates a generative AI layer to provide automated market insights based on real time database snapshots.

\begin{itemize}
    \item \textbf{Inputs to the Agent:}
    The system constructs a grounded prompt by retrieving the following data from the PostgreSQL instance:
    \begin{itemize}[nosep]
        \item \textbf{Rental Data:} Latest average monthly rent, period, and annual change from \texttt{rental\_metrics}.
        \item \textbf{HPI Data:} The last 6 months of House Price Index trends for existing builds in the specified region.
        \item \textbf{Affordability:} The latest price to income ratio for all dwellings.
        \item \textbf{Demographics:} Region specific implied average borrower income and corresponding year.
        \item \textbf{Contextual Overlays:} Optional user provided parameters including $salary$, $focus$ (investor, first-time buyer), and $property\_type$.
    \end{itemize}
    
    \item \textbf{Prompt Structure:} 
    A system level instruction defines the agent as a UK real estate analyst. This is followed by a structured list of the retrieved figures. The model is strictly constrained to use only the provided data points to generate a factual, 150 word executive summary, preventing hallucinations or the invention of figures.

    \item \textbf{Expected Agent Output:}
    The model returns a single string of plain text containing the market health summary. No additional formatting or JSON structure is requested from the model to simplify parsing.

    \item \textbf{API Response to Client:}
    The controller synthesizes the AI text into a standard JSON response:
    \begin{itemize}[nosep]
        \item \texttt{region}: Metadata including $id$, $name$, and $gss\_code$.
        \item \texttt{summary}: The plaintext string generated by Gemini.
        \item \texttt{data\_snapshot}: The raw numerical values used to ground the prompt.
        \item \texttt{generated\_at}: Timestamp of the request.
    \end{itemize}
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{503 Service Unavailable}: Returned if the Gemini API fails.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{ONS Rent to salary ratios}: \href{https://www.ons.gov.uk/peoplepopulationandcommunity/housing/bulletins/privaterentalaffordabilityengland/2024}{[Link to Bulletin]}
    \textit{Accessed: March 10--16, 2026}
